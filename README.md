...
Open the ml/ directory on your machine.
Since you already installed the dependencies in Step 1, you can directly run the Jupyter Notebooks in the following order:
...
If you want to update the backend's active lookup table with your freshly trained data, make sure the generated locality_aggregates.json is present in the root directory.
3. Backend Setup & Run
...
Start Backend Server: Ensure your virtual environment is still active, then run:
...
npx expo start -c
3. AI Setup (Optional)
To use the local fallback, install Ollama and run:

ollama run gemma3:1b
📈 The Safety Logic
The Safety Index (0-100) is calculated as: Base Score (Dataset) × Lux Penalty (Live Sensor) = Final Safety Score
